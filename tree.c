...
tnode* tnode_create(const char* word) {
  ...
  p->word = _strdup(word);    // copy of word allocated on heap
  p->count = 1;
  p->left = NULL;
  p->right = NULL;
...
void fileInput(tree* t, const char* filename) {
...
void consoleInput(tree* t, int argc, const char* argv[]) {
	int i = 1;
	char* p = strtok((char*)argv[i++], ",. !/\\;:\"\'<>?`~!@#$%^&*()_=+");
	tree_add(t, p);

	while (p != NULL && i < argc) {
		p = strtok((char*)argv[i], ",. !/\\;:\"\'<>?`~!@#$%^&*()_=+");
		tree_add(t, p);
		++i;
	}
}
...
int main(int argc, const char* argv[]) {
	...
	if (argc == 1) {
		fprintf(stderr, "Usage: ./program file OR more than console input of 2 or more words\n");
		exit(1);
	}
	else if (argc == 2) {
		fileInput(t, argv[1]);
